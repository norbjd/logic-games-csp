...
public class GaramCSP {
    ...
    private List<Constraint> twoDigitsNumbersConstraints() {
        return Arrays.asList(
                model.arithm(twoDigitsNumbers[0], "=", cells[11].mul(10).add(cells[15]).intVar()),
    ...
    private List<Constraint> horizontalEquationsConstraints() {
        return Arrays.asList(
                equation(cells[0],  garam.getOperator(0),   cells[1],     cells[2]),
    ...
    private List<Constraint> verticalEquationsConstraints() {
        return Arrays.asList(
                equation(cells[0],  garam.getOperator(2),   cells[6],   twoDigitsNumbers[0]),
    ...
    private List<Constraint> presetValuesConstraints() {
        ...
        Stream<Pair<IntVar, Integer>> zippedCellsWithPresetValues = zippedCellsWithValues.filter(
                (Pair<IntVar, Integer> pair) -> pair.getRight() != -1
        );
